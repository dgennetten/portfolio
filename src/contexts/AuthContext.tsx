...
import {
  AUTH_TOKEN_KEY, AUTH_SESSION_KEY, AUTH_EXPIRES_KEY, AUTH_REMEMBER_KEY,
...
function loadSession(): AuthUser | undefined {
  try {
    ...
    if (expires && Date.now() > Number(expires)) {
      clearSession(localStorage.getItem(AUTH_REMEMBER_KEY) === '1');
      return undefined;
    }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback((
    ...
    remember: boolean, expiresAtMs?: number,
  ) => {
